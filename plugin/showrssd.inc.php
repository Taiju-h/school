...
// 表示行数も選択できるようにした
//
// 2005/07/07 modified by oak <oak@bddb.org>
// パインさんの表示桁数を調節する部分をDESCRIPTIONの表示非表示のスイッチに変えています。
// あと、タイトルと一緒に編集の日時を表示するように手を加えました。


// showrssプラグインが使用可能かどうかを表示
function plugin_showrssd_action()
{
...
function plugin_showrssd_convert()
{
	if (func_num_args() == 0)
	{
		// 引数がない場合はエラー
	...
	list($rss,$time) = plugin_showrssd_get_rss($rssurl,$usecache);
	if ($rss === FALSE)
	{
...
class ShowRSS_html
{
	...
	function ShowRSS_html($rss,$showlines)
	{
		...
		foreach ($rss as $date=>$items)
		{
			...
			{
				if($showlines and $showlines <= $i){ break; }
				$link = $item['LINK'];
				$title = $item['TITLE'];
				$idate = $item['DC:DATE'];
...
// rssを取得する
function plugin_showrssd_get_rss($target,$usecache)
{
	$buf = '';
	$time = NULL;
	if ($usecache)
	{
		// 期限切れのキャッシュをクリア
		plugin_showrssd_cache_expire($usecache);
...
// 期限切れのキャッシュをクリア
function plugin_showrssd_cache_expire($usecache)
{
...
class ShowRSS_XML
{
	...
	function end_element($parser,$name)
	{
		...
		{
			$time = plugin_showrssd_get_timestamp($item['DC:DATE']);
		}
		else if (array_key_exists('PUBDATE',$item))
		{
			$time = plugin_showrssd_get_timestamp($item['PUBDATE']);
		}
...
function plugin_showrssd_get_timestamp($str)
{
	if (($str = trim($str)) == '')
	{
		return UTIME;
